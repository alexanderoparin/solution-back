...
import ru.oparin.solution.service.WbArticleNoteService;
...
@Slf4j
public class WbArticleNotesController {

    private final WbArticleNoteService noteService;
     ...
     */
    @PostMapping
    public ResponseEntity<WbArticleNoteDto> createNote(
        ...
        WbArticleNoteDto note = noteService.createNote(nmId, context, request, currentUser);
     ...
     */
    @GetMapping
    public ResponseEntity<List<WbArticleNoteDto>> getNotes(
            @PathVariable Long nmId,
            @RequestParam(required = false) Long sellerId,
            @RequestParam(required = false) Long cabinetId,
            Authentication authentication) {
        ...
        SellerContextService.SellerContext context = sellerContextService.createContext(authentication, sellerId, cabinetId, ru.oparin.solution.model.CabinetAccessSection.PRODUCTS);
        List<WbArticleNoteDto> notes = noteService.getNotes(nmId, context);
     ...
     */
    @PutMapping("/{noteId}")
    public ResponseEntity<WbArticleNoteDto> updateNote(
            @PathVariable Long nmId,
        ...
        User currentUser = userService.findByEmail(authentication.getName());
        SellerContextService.SellerContext context = sellerContextService.createContext(authentication, sellerId, cabinetId, ru.oparin.solution.model.CabinetAccessSection.PRODUCTS);
        WbArticleNoteDto note = noteService.updateNote(noteId, nmId, context, request, currentUser);
    ...
    public ResponseEntity<ru.oparin.solution.dto.notes.WbArticleNoteFileDto> uploadFile(
            @PathVariable Long nmId,
            @PathVariable Long noteId,
            @RequestParam(required = false) Long sellerId,
            @RequestParam(required = false) Long cabinetId,
        ...
        User currentUser = userService.findByEmail(authentication.getName());
        SellerContextService.SellerContext context = sellerContextService.createContext(authentication, sellerId, cabinetId, ru.oparin.solution.model.CabinetAccessSection.PRODUCTS);
        ru.oparin.solution.dto.notes.WbArticleNoteFileDto fileDto = noteService.uploadFile(noteId, nmId, context, file, currentUser);
    ...
    public ResponseEntity<Resource> downloadFile(
        ...
        ru.oparin.solution.dto.notes.WbArticleNoteFileDto fileInfo = noteService.getFileInfo(noteId, fileId, nmId, context);
