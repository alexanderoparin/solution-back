...
import ru.oparin.solution.service.WbCampaignNoteService;
...
public class WbCampaignNotesController {

    private final WbCampaignNoteService noteService;
    private final SellerContextService sellerContextService;
    private final UserService userService;

    @GetMapping
    public ResponseEntity<List<WbCampaignNoteDto>> getNotes(
        ...
        List<WbCampaignNoteDto> notes = noteService.getNotes(campaignId, context);
        return ResponseEntity.ok(notes);
    }

    @PostMapping
    public ResponseEntity<WbCampaignNoteDto> createNote(
            @PathVariable Long campaignId,
            @RequestParam(required = false) Long sellerId,
            @RequestParam(required = false) Long cabinetId,
        ...
        SellerContextService.SellerContext context = sellerContextService.createContext(authentication, sellerId, cabinetId, ru.oparin.solution.model.CabinetAccessSection.AD_CAMPAIGNS);
        WbCampaignNoteDto note = noteService.createNote(campaignId, context, request, currentUser);
        return ResponseEntity.status(HttpStatus.CREATED).body(note);
    }

    @PutMapping("/{noteId}")
    public ResponseEntity<WbCampaignNoteDto> updateNote(
            @PathVariable Long campaignId,
        ...
        User currentUser = userService.findByEmail(authentication.getName());
        SellerContextService.SellerContext context = sellerContextService.createContext(authentication, sellerId, cabinetId, ru.oparin.solution.model.CabinetAccessSection.AD_CAMPAIGNS);
        WbCampaignNoteDto note = noteService.updateNote(noteId, campaignId, context, request, currentUser);
    ...
    public ResponseEntity<WbCampaignNoteFileDto> uploadFile(
            @PathVariable Long campaignId,
            @PathVariable Long noteId,
            @RequestParam(required = false) Long sellerId,
            @RequestParam(required = false) Long cabinetId,
        ...
        User currentUser = userService.findByEmail(authentication.getName());
        SellerContextService.SellerContext context = sellerContextService.createContext(authentication, sellerId, cabinetId, ru.oparin.solution.model.CabinetAccessSection.AD_CAMPAIGNS);
        WbCampaignNoteFileDto dto = noteService.uploadFile(noteId, campaignId, context, file, currentUser);
    ...
    public ResponseEntity<Resource> downloadFile(
        ...
        WbCampaignNoteFileDto fileInfo = noteService.getFileInfo(noteId, fileId, campaignId, context);
