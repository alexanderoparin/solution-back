 ...
 */
@Repository
public interface WbPromotionNormQueryStatisticsRepository
        extends JpaRepository<WbPromotionNormQueryStatistics, Long>, WbPromotionNormQueryStatisticsRepositoryCustom {

    @Modifying
    @Query("DELETE FROM WbPromotionNormQueryStatistics s WHERE s.campaign.advertId IN :campaignIds "
    ...
    @Query("SELECT s.id FROM WbPromotionNormQueryStatistics s WHERE s.campaign.cabinet.id = :cabinetId")
    List<Long> findIdByCampaign_Cabinet_Id(@Param("cabinetId") Long cabinetId, Pageable pageable);

    @Query("SELECT MAX(s.updatedAt) FROM WbPromotionNormQueryStatistics s "
