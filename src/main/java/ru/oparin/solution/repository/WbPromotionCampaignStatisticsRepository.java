 ...
 */
@Repository
public interface WbPromotionCampaignStatisticsRepository extends JpaRepository<WbPromotionCampaignStatistics, Long> {
     ...
     */
    Optional<WbPromotionCampaignStatistics> findByCampaignAdvertIdAndDate(Long campaignId, LocalDate date);
     ...
     * @return статистика или пусто
     */
    Optional<WbPromotionCampaignStatistics> findByCampaignAdvertIdAndNmIdAndDate(
            Long campaignId,
            Long nmId,
            LocalDate date
    );
     ...
     */
    List<WbPromotionCampaignStatistics> findByCampaignAdvertIdAndDateBetween(
            Long campaignId,
            LocalDate dateFrom,
            LocalDate dateTo
    );
    ...
    List<WbPromotionCampaignStatistics> findByCampaignAdvertIdInAndDateBetween(
    ...
    @Query("SELECT s FROM WbPromotionCampaignStatistics s " +
           "WHERE s.nmId = :nmId " +
           "AND s.date BETWEEN :dateFrom AND :dateTo")
    List<WbPromotionCampaignStatistics> findByNmIdAndDateBetween(
     ...
     */
    @Query("SELECT s FROM WbPromotionCampaignStatistics s "
            + "WHERE s.campaign.advertId = :campaignAdvertId "
            + "AND s.nmId = :nmId "
            + "AND s.date BETWEEN :dateFrom AND :dateTo")
    List<WbPromotionCampaignStatistics> findByCampaignAdvertIdAndNmIdAndDateBetween(
            @Param("campaignAdvertId") Long campaignAdvertId,
            @Param("nmId") Long nmId,
            @Param("dateFrom") LocalDate dateFrom,
            @Param("dateTo") LocalDate dateTo
    );
     ...
     * @return список nm_id
     */
    @Query("SELECT DISTINCT s.nmId FROM WbPromotionCampaignStatistics s WHERE s.campaign.advertId = :campaignId")
     ...
     */
    @Query("SELECT s.id FROM WbPromotionCampaignStatistics s WHERE s.campaign.cabinet.id = :cabinetId")
