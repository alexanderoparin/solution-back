 ...
 */
@Repository
public interface WbProductCardRepository extends JpaRepository<WbProductCard, Long> {

    /**
     * Находит карточку по nmID.
     */
    Optional<WbProductCard> findByNmId(Long nmId);

    /**
     * Находит все карточки продавца (по всем его кабинетам).
     */
    List<WbProductCard> findByCabinet_User_Id(Long userId);

    /**
     * Находит все карточки кабинета.
     */
    List<WbProductCard> findByCabinet_Id(Long cabinetId);

    /**
     * Выборка только ключей (nmId) по кабинету пачкой (для пакетного удаления по ключам).
     */
    @Query("SELECT c.nmId FROM WbProductCard c WHERE c.cabinet.id = :cabinetId")
     ...
     */
    List<WbProductCard> findByImtIdAndCabinet_Id(Long imtId, Long cabinetId);
