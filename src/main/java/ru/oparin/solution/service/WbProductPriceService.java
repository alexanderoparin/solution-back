...
import ru.oparin.solution.repository.WbProductPriceHistoryRepository;
...
@Slf4j
public class WbProductPriceService {
     ...
     */
    @Transactional
    public void savePrices(WbProductPricesResponse response, LocalDate yesterdayDate, Cabinet cabinet) {
        if (response == null || response.getData() == null || response.getData().getListGoods() == null) {
            log.warn("Получен пустой ответ с ценами товаров");
            return;
        }

        List<WbProductPriceHistory> built = new ArrayList<>();
        int skippedCount = 0;

        for (WbProductPricesResponse.Good good : response.getData().getListGoods()) {
            if (!isValidGood(good)) {
        ...
        Map<NmSizeKey, WbProductPriceHistory> incomingByKey = dedupeByNmAndSize(built);
        List<Long> nmIds = incomingByKey.keySet().stream().map(NmSizeKey::nmId).distinct().toList();
        Map<NmSizeKey, WbProductPriceHistory> existingByKey = loadExistingPriceMap(nmIds, yesterdayDate, cabinet.getId());

        List<WbProductPriceHistory> toPersist = new ArrayList<>();
        for (WbProductPriceHistory incoming : incomingByKey.values()) {
            NmSizeKey key = NmSizeKey.of(incoming);
            WbProductPriceHistory row = existingByKey.get(key);
            if (row != null) {
    ...
    private void alignSppDiscountAcrossSizeRows(List<Long> nmIds, LocalDate date, Long cabinetId) {
        ...
        for (Long nmId : new LinkedHashSet<>(nmIds)) {
            List<WbProductPriceHistory> rows = priceHistoryRepository.findByNmIdAndDateAndCabinet_Id(nmId, date, cabinetId);
            if (rows.size() < 2) {
                continue;
            }
            Integer spp = rows.stream()
                    .map(WbProductPriceHistory::getSppDiscount)
            ...
            for (WbProductPriceHistory row : rows) {
    ...
    private static Map<NmSizeKey, WbProductPriceHistory> dedupeByNmAndSize(List<WbProductPriceHistory> rows) {
        Map<NmSizeKey, WbProductPriceHistory> map = new LinkedHashMap<>();
        for (WbProductPriceHistory row : rows) {
    ...
    private Map<NmSizeKey, WbProductPriceHistory> loadExistingPriceMap(List<Long> nmIds, LocalDate date, Long cabinetId) {
        List<WbProductPriceHistory> existing = priceHistoryRepository.findByNmIdInAndDateAndCabinet_Id(nmIds, date, cabinetId);
        Map<NmSizeKey, WbProductPriceHistory> map = new HashMap<>();
        for (WbProductPriceHistory row : existing) {
            map.put(NmSizeKey.of(row), row);
        }
        return map;
    }
     ...
     */
    private static void applyPriceFieldsFromApi(WbProductPriceHistory target, WbProductPriceHistory source) {
    ...
    private record NmSizeKey(Long nmId, Long sizeId) {
        static NmSizeKey of(WbProductPriceHistory p) {
            return new NmSizeKey(p.getNmId(), p.getSizeId());
        }
    }

    private boolean isValidGood(WbProductPricesResponse.Good good) {
        return good != null
                && good.getNmId() != null
    ...
    private List<WbProductPriceHistory> processGood(WbProductPricesResponse.Good good, LocalDate date, Cabinet cabinet) {
        List<WbProductPriceHistory> prices = new ArrayList<>();

        if (areAllPricesEqual(good.getSizes())) {
            WbProductPriceHistory price = createPriceHistory(good, date, null, null, good.getSizes().get(0), cabinet);
            prices.add(price);
        } else {
            for (WbProductPricesResponse.Size size : good.getSizes()) {
                if (isValidSize(size)) {
                    WbProductPriceHistory price = createPriceHistory(good, date, size.getSizeId(), size.getTechSizeName(), size, cabinet);
                    prices.add(price);
                }
            }
        }
    ...
    private boolean areAllPricesEqual(List<WbProductPricesResponse.Size> sizes) {
        if (sizes == null || sizes.isEmpty() || sizes.size() == 1) {
            return true;
        }

        WbProductPricesResponse.Size firstSize = sizes.get(0);
    ...
    private boolean isValidSize(WbProductPricesResponse.Size size) {
        return size != null
                && size.getPrice() != null
    ...
    private WbProductPriceHistory createPriceHistory(
            WbProductPricesResponse.Good good,
            LocalDate date,
            Long sizeId,
            String techSizeName,
            WbProductPricesResponse.Size size,
            Cabinet cabinet
    ) {
        return WbProductPriceHistory.builder()
                .nmId(good.getNmId())
                .cabinet(cabinet)
                .date(date)
                .sizeId(sizeId)
     ...
     */
    public List<WbProductPriceHistory> getCurrentPrices(Long nmId, LocalDate yesterdayDate) {
     ...
     */
    public List<WbProductPriceHistory> getPricesByNmIdsAndDate(List<Long> nmIds, LocalDate date, Long cabinetId) {
     ...
     */
    public List<WbProductPriceHistory> getPricesByNmIdsAndDate(List<Long> nmIds, LocalDate date) {
    ...
    public void updateSppDiscount(java.util.Map<Long, Integer> sppByNmId, LocalDate date, Long cabinetId) {
        ...
        for (java.util.Map.Entry<Long, Integer> entry : sppByNmId.entrySet()) {
            ...
            List<WbProductPriceHistory> prices = cabinetId != null
            ...
            for (WbProductPriceHistory price : prices) {
