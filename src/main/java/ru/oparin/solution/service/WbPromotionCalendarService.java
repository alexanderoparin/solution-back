...
import ru.oparin.solution.repository.WbPromotionParticipationRepository;
...
@Service
@Slf4j
public class WbPromotionCalendarService {

    private final WbCalendarApiClient calendarApiClient;
    private final WbProductCardRepository productCardRepository;
    private final WbPromotionParticipationRepository participationRepository;
    private final CabinetService cabinetService;
    private final CabinetScopeStatusService cabinetScopeStatusService;
    private final WbPromotionCalendarService self;

    public WbPromotionCalendarService(WbCalendarApiClient calendarApiClient,
                                    WbProductCardRepository productCardRepository,
                                    WbPromotionParticipationRepository participationRepository,
                                    CabinetService cabinetService,
                                    CabinetScopeStatusService cabinetScopeStatusService,
                                    @Lazy WbPromotionCalendarService self) {
    ...
    public void syncPromotionsForCabinet(Cabinet cabinet) {
        ...
        try {
            WbCalendarPromotionsResponse response = calendarApiClient.getPromotions(apiKey, start, end, false);
            ...
            List<WbPromotionParticipation> toSave = new ArrayList<>();
            Set<String> uniqueKeys = new HashSet<>();

            for (WbCalendarPromotionsResponse.CalendarPromotionItem promo : response.getData().getPromotions()) {
                ...
                for (Long nmId : inPromotion) {
                    if (cabinetNmIdSet.contains(nmId)) {
                        ...
                        toSave.add(WbPromotionParticipation.builder()
                                .cabinet(cabinet)
                                .nmId(nmId)
