...
import ru.oparin.solution.service.WbProductPriceService;
...
public class WbProductPricesSyncService {
    ...
    public void updateSppFromOrders(Cabinet cabinet, String apiKey) {
        ...
        try {
            List<WbOrdersResponse.Order> orders = ordersApiClient.getOrders(apiKey, date, 1);
    ...
    public void loadPricesBatch(Cabinet cabinet, String apiKey, LocalDate date, List<Long> nmIdsBatch) {
        ...
        WbProductPricesRequest request = WbProductPricesRequest.builder().nmList(nmIdsBatch).build();
        WbProductPricesResponse response = productsApiClient.getProductPrices(apiKey, request);
    ...
    private List<Long> getDistinctNmIdsForCabinet(Long cabinetId) {
        List<WbProductCard> productCards = productCardRepository.findByCabinet_Id(cabinetId);
    ...
    private Set<Long> getExistingNmIdsForDate(List<Long> nmIds, LocalDate date, Long cabinetId) {
        List<WbProductPriceHistory> existingPrices = productPriceService.getPricesByNmIdsAndDate(nmIds, date, cabinetId);
        return existingPrices.stream()
                .map(WbProductPriceHistory::getNmId)
    ...
    private void loadPricesInBatches(Cabinet cabinet, String apiKey, LocalDate date, List<Long> nmIdsToLoad) {
        ...
        for (int i = 0; i < batches.size(); i++) {
            ...
            try {
                WbProductPricesRequest request = WbProductPricesRequest.builder().nmList(batch).build();
                WbProductPricesResponse response = productsApiClient.getProductPrices(apiKey, request);
                productPriceService.savePrices(response, date, cabinet);
            } catch (WbApiUnauthorizedScopeException e) {
                handleWbUnauthorizedScope(cabinet, e);
    ...
    private static SppExtractionResult extractSppByNmIdFromOrders(List<WbOrdersResponse.Order> orders) {
        Map<Long, Integer> sppLastValueByNmId = new HashMap<>();
        Map<Long, Set<Integer>> sppValuesByNmId = new HashMap<>();

        for (WbOrdersResponse.Order order : orders) {
