...
import ru.oparin.solution.service.campaign.WbCampaignStartBudgetGuard;
...
@Slf4j
public class WbPromotionCampaignControlService {
    ...
    private static final Set<WbCampaignStatus> START_ALLOWED = Set.of(
            WbCampaignStatus.READY_TO_START,
            WbCampaignStatus.PAUSED
    );
    private static final Set<WbCampaignStatus> PAUSE_ALLOWED = Set.of(WbCampaignStatus.ACTIVE);
    ...
    public ScheduleControlAttemptResult attemptStartFromSchedule(Cabinet cabinet, Long advertId) {
        try {
            ...
            WbPromotionCampaign campaign = findCampaignOrThrow(cabinet.getId(), advertId);
    ...
    private CampaignControlEnqueueResponse enqueueStart(
        ...
        Long eventId = wbApiEventService.enqueueWbPromotionCampaignStart(cabinet.getId(), advertId, triggerSource);
        if (eventId == null) {
    ...
    private CampaignControlEnqueueResponse enqueuePause(
        ...
        Long eventId = wbApiEventService.enqueueWbPromotionCampaignPause(cabinet.getId(), advertId, triggerSource);
        if (eventId == null) {
            return new CampaignControlEnqueueResponse(
    ...
    private WbPromotionCampaign findCampaignOrThrow(Long cabinetId, Long advertId) {
    ...
    private void validateStatusForStart(WbPromotionCampaign campaign) {
        WbCampaignStatus status = campaign.getStatus();
    ...
    private void validateStatusForPause(WbPromotionCampaign campaign) {
        WbCampaignStatus status = campaign.getStatus();
    ...
    private ScheduleControlAttemptResult handleUnauthorizedForSchedule(
        ...
        if (WbPromotionCampaignControlWriteService.isReadOnlyTokenError(exception)) {
            promotionControlWriteService.recordReadOnlyTokenBlock(cabinet.getId());
            return ScheduleControlAttemptResult.failed(WbPromotionCampaignControlWriteService.READ_ONLY_USER_MESSAGE);
        }
