 ...
 * - CPC определяется отдельно по settings.payment_type и имеет приоритет в интерфейсе
 * 
...
public enum WbCampaignType {
     ...
     */
    public static WbCampaignType fromCode(Integer code) {
        if (code == null) {
            return null;
        }
        for (WbCampaignType type : values()) {
